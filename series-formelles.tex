...
This is the paper by Joyal that introduced the concept of
\emph{combinatorial species}, as a way to unify and generalize the
existing theory of generating functions.  It was one of the first
attempts to apply category theory to combinatorics---now a proven
combination.
...
read a lot of secondary literature on species and assumed that would
have to be enough, since I do not read French---the language in which
Joyal's paper was written.  However, I eventually came across some
questions to which I could not find answers, only to be told that the
answers were already in Joyal's paper!  Somewhat reluctantly I found a
copy and began trying to read it, whereupon I discovered several
surprising things.

First of all, reading mathematical French is not too difficult, even
for someone who does not know any French, especially if one already
understands the math---although it is certainly slow going.  Second,
it turns out that Joyal's is a fantastic paper full of interesting
ideas and examples which did not appear in any of the secondary
literature.

I am excited to bring this work to the attention of the functional
programming languages community, but there are two barriers to it
being more widely accessible in that context: the language barrier is
the obvious one, but more importantly, it assumes a lot of background
knowledge (in mathematics in general and combinatorics in particular)
which many people in the programming languages community may not
have.  To address the first barrier, I decided to produce an English translation---to address the first
barrier---and to add commentary, examples, and code to address the
second.  The commentary will be typeset like this:
\begin{commentary}
  This is some commentary.
\end{commentary}
This is a long-term project; I do not know how long it will take to
finish, but plan to work on it slowly and steadkly.  Collaboration and
contributions are welcome!  Things I could particularly use help with:
\begin{itemize}
\item Translation.  If you know both French and English well, I would
  love more eyes on the translation.  There are particular places
  where I am not sure about the translation, which I have indicated
  \trans{like this}{comme \c{c}a}.  However, you need not limit
  yourself to looking at those places; all of the translation could
  use checking.
\item Commentary. I would of course be happy to receive contributions
  ...
  parts you find confusing or unclear---that will be a signal that the
