...
\begin{commentary}
  ...
  without things like products or coproducts.  \todo{Expand this
    explanation.  Natural transformations have to satisfy naturality
    square, every side of which is an arrow in $\B$, so only natural
    isomorphisms.}  Natural transformations between functors
  $\B \to \E$, on the other hand, need not be isomorphisms, and the
  functor category $\E^\B$ indeed admits nice constructions such as
  products and coproducts, which turn out to correspond to sensible
  operations on species.  Hence $\B \to \E$ really seems to be the
  ...
  many years of thinking about such things!)
\end{commentary}
