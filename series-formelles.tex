...
\begin{commentary}
  ...
  that is, under relabelling.  Intuitively, a type is a \emph{shape}
  devoid of any labels.

  For example, XXX use example related to trees $s$ and $t$ from
  before.  Show a bunch of trees isomorphic to $s$ and show

  Intuitively, we can think of types in terms of these pictures of
  ``shapes'' with nondescript dots in place of labels.  This isn't
  always good enough---for exampe, XXX---so it's good to be able to
