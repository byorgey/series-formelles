...
dia = (drawBinTreeWide . fmap (const (circle labR # fc black))) s
  ...
  $\el (M)$ is of course a \term{groupoid} because isomorphisms
  between $M$-structures are invertible: if $u : s \to t$, that is, if
  $u$ is a bijection between label sets such that relabelling the
  structure $s$ by $u$ yields $t$, then we necessarily have
  $u^{-1} : t \to s$.
\end{commentary}
...
\begin{commentary}
  Essentially, simplicial schemes can be thought of as a
  generalization of graphs which allows not only for points $\{u\}$
  (0-dimensional simplices) and edges $\{u,v\}$ (1-dimensional
  simplices) but also ``filled-in'' triangles $\{u,v,w\}$, tetrahedra,
  and so on.  These really don't come up again in the paper; the
  important point is that we have a species of graphs (undirected,
  ...
  distinguished root node; this will be explored in much more detail
  later.
...
\begin{commentary}
  ...
  and $\E^\B$ are not equivalent.  Although we have seen above that
  these two categories have essentially the same collection of
  ...
  morphisms than that of isomorphisms'' (something of an
  understatement).  Hence $\B \to \E$ really seems to be the ``right''
  notion of species to use.  (I am grateful to Ian Price for pointing
  out this fact to me, which I had somehow missed even after many
  years of thinking about such things!  And of course, it was in Joyal
  all along.)
\end{commentary}
