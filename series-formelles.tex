...
During the course of researching my PhD dissertation on the
intersection of combinatorial species and programming languages, I
read a lot of secondary literature on species, but not Joyal's
original paper---it is written in French, which I do not read. I
didn't think this would be a great loss, since I supposed the material
in his paper would be well-covered elsewhere, for example in the
textbook by \citet{bll} (which thankfully \emph{has} been translated
into English). However, I eventually came across some questions to
which I could not find answers, only to be told that the answers were
already in Joyal's paper!  Somewhat reluctantly I found a copy and
began trying to read it, whereupon I discovered several surprising
things.
...
understands the math.  Second, it turns out that Joyal's paper makes
for excellent reading, and is full of insights and examples which do
not appear in any of the secondary literature.

\todo{some sort of transition saying why/when I decided to do this}

Joyal's paper (and the theory of species more generally) has a lot to
offer to computer science, and to functional programming in
particular, but there are two primary barriers preventing it from
being widely accessible in that context. The language barrier is the
obvious one, which is easily enough addressed by a translation.  The
second, more serious barrier is that the paper assumes background
knowledge (in mathematics in general and combinatorics in particular)
which many in the programming languages community may not have.  To
address this barrier I have added commentary, examples, and code that
help to explain and illuminate the content to a wider audience.  The
commentary is typeset like this:
\begin{commentary}
...
contributions are welcome!  Things I could particularly use help with include:
\begin{itemize}
...
\item Commentary. I would of course be happy to receive contributions
  of additional commentary. But even (especially!) if you are not an
  expert, an incredibly helpful way you can contribute is simply to
  try reading the translation and commentary, and let me know of any
  parts you find confusing or unclear---a signal that the
  commentary needs to be clarified, or more commentary added.
\end{itemize}

I am also interested to eventually publish this somehow---if you
...
The original paper is available only in a scanned form, so as a first
step I ran it through the tesseract OCR engine
(\url{https://github.com/tesseract-ocr/tesseract}). \todo{Google translation toolkit. LaTeX. diagrams.}
...
\item Necessary background:
  \begin{itemize}
  \item Very basic category theory (categories, functors)
  \end{itemize}
\end{itemize}
...
\begin{commentary}
  ...
  much more elegant and economical way.  Interested readers can see
  Bourbaki's definition in Appendix~\ref{sec:appendix-bourbaki}.
\end{commentary}
...
\begin{commentary}
  Recall that a \emph{groupoid} is a category where all the morphisms
  are ``invertible'', that is, each $m : A \to B$ has a corresponding
  $m^{-1} : B \to A$ such that $m^{-1} \comp m = \id_A$ and $m \comp
  m^{-1} = \id_B$.  $\B$ is the category whose objects are
  ...
  It is always possible to make a bijection between any two finite
  sets of the same size; conversely, there are no bijections between
  finite sets of different sizes.  Thus, $\B$ can be thought of as the
  disjoint union of a number of connected components, one for each
  natural number size.

  Recall that an endofunctor $\B \to \B$ has two components: a map XXX
  and XXX, satisfying some laws.  objects to objects (in this case,
  sets of labels to sets of structures) and morphisms to
  morphisms (in this case, bijections between label sets to bijections
  between labelled structures). Intuitively, we think of XXX
...
\begin{commentary}
  ...
  XXX crux of the idea.  No complicated structural definition.  Just
...
The basic idea is that the variables $x_1, \dots, x_n$ represent sets
containing labels, and the variables $s_1, \dots, s_p$ represent
structures.  The relation $R\{x_1, \dots, x_n, s_1, \dots, s_p\}$
holds precisely when the $s_j$ are all the (multi-sorted) structures
that can be built out of the labels in the $x_i$.  Condition (IS)
ensures that we can bijectively swap out the label sets $x_i$ for
different ones.
