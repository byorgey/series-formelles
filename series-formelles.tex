...
\begin{commentary}
  ...
  introduces functors $\B \to \E$ later, in
  \pref{sec:category-of-species}.

  When considering an individual species, the distinction does not
  seem important: since functors necessarily preserve isomorphisms,
  every bijection in $\B$ must actually map to a bijection in $\E$, so
  the image of any functor $\B \to \E$ actually lies in $\B$.
  However, considering the whole collection of species as a category,
  the distinction is an important one: the functor categories $\B^\B$
  and $\E^\B$ are not equivalent.  Although these two categories have
  essentially the same collection of \emph{objects} (namely, functors
  $\B \to \B$ or $\B \to \E$), they do not have the same
  \emph{morphisms} (namely, natural transformations).  Any natural
  transformation between functors $\B \to \B$ is an isomorphism, so
  the functor category $\B^\B$ is a rather impoverished groupoid,
  without things like products or coproducts.  Natural transformations
  between functors $\B \to \E$, on the other hand, need not be
  isomorphisms, and the functor category $\E^\B$ indeed admits nice
  constructions such as products and coproducts, which turn out to
  correspond to sensible operations on species.  Hence $\B \to \E$
  really seems to be the ``right'' notion of species to use.  (I am
  grateful to Ian Price for pointing out this fact to me, which I had
  somehow missed even after many years of thinking about such things!)
\end{commentary}
...
\begin{commentary}
  ...
  general in some sense, but the point is that this allows Joyal to
  deal in one fell swoop with a large collection of common species
  (graphs, forests, trees\dots) which can all be seen as special cases
  of the species of simplicial schemes, and also incidentally to have
  a relevant motivation for introducing the concept of subspecies.  It
  would be tedious to go through each slight variant on the concept of
  a graph and show that it defines a valid species, but unsatisfying
  to just wave our hands and say things like ``this case is
  similar\dots''  By moving up a notch in abstraction, we get a
...
\begin{ex} \label{ex:endofunctions}
  ...
  \begin{commentary}
    ...
    XXX pictures illustrating permutations, connected endofunctions,
