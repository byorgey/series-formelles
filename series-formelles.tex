...
\begin{commentary}
  ...
  general in some sense, but it is a nice observation.  Geometrically,
  \term{simplices} are what we get when we start with a single point
  (a ``$0$-simplex''), and then repeatedly add points, each time
  connecting the new point to all the previous points (and also
  ``filling in'' the interior).  For example, a $1$-simplex is a line
  segment between two points; a $2$-simplex is a (filled in) triangle;
  a $3$-simplex is a solid tetrahedron; and after that they get harder
  to visualize, but conceptually are just the natural generalization
  of triangles and tetrahedra to $n$ dimensions. Notice that an
  $n$-simplex has a ``boundary'' which consists of ($n-1$)-simplices: a
  line segment has two endpoints; a triangle has three edges; a
  tetrahedron has four triangular faces. (And yes, a four-dimensional
  $4$-simplex has five tetrahedral ``faces''.)
  \begin{center}
  \begin{diagram}[width=200]
dia =
  [ pt
  ...
  A \term{simplicial complex}, intuitively, is just a bunch of
  simplices glued together in some way.

  Often we don't care about the actual geometry, but only the way in
  which

  Simplicial scheme on a set of vertices $E$ is a set of non-empty
  subsets; we think of each subset as specifying that the vertices in
  the subset are all connected by an appropriate simplex.  For
  example, \dots  The condition that every subset of some element of
  $\Simp$ is also in $\Simp$ is just saying that a simplex must
  contain all its faces.  XXX explain this better.
...
dia = mconcat
  ...
  , (hrule 1 # alignL <> pt) # alignR # rotate (20 @@ deg)
  ]
  # frame 0.5
  # lwO 0.7
  where
  ...
  XXX talk about specific subspecies he gives.
\end{commentary}
\end{ex}
