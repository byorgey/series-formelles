...
paper!\footnote{\url{http://mathoverflow.net/questions/171452/examples-of-functors-mathbfset-to-mathbfset-which-are-not-analytic}}
Somewhat reluctantly, I found a copy and began trying to read it,
whereupon I discovered several surprising things.
...
for excellent reading, and is full of insights and examples which, as
far as I know, do not appear in any of the secondary literature.  The
paper (and the theory of species more generally) has a lot to offer to
computer science, and to functional programming in particular.
...
The commentary is typeset like this:
\begin{commentary}
...
(\url{https://github.com/tesseract-ocr/tesseract}), which does an
excellent job with the text, although naturally it completely mangles
the diagrams and equations.  From there I have loaded the resulting
text document into the Google Translator Toolkit, and use that to do
the bulk of the translating.  The result gets pasted into a \LaTeX\
document, at which point I re-add equations and diagrams, proofread,
and add my own commentary.  The diagrams are all produced using the
\texttt{diagrams} vector graphics framework
