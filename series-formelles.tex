...
\begin{commentary}
  ...
  ``filling in'' the interior).  For example, a $1$-simplex is a
  ...
  get harder to visualize, but conceptually are just like
  ``$n$-dimensional tetrahedra''.  Notice that an $n$-simplex has a
  ``boundary'' which consists of $n-1$ simplices: a line segment has
  two endpoints; a triangle has three edges; a tetrahedron has four
  triangular faces. (And yes, a four-dimensional $4$-simplex has five
  tetrahedral ``faces''.)
  \begin{center}
  \begin{diagram}[width=200]
dia =
