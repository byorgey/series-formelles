...
\begin{align*}
  ...
  |F \cdot G| &= |F| \cdot |G|,
\end{align*}
where we use $|F|$ to denote the size (\ie number of structures) of a
finite combinatorial class.  That is, the size of a disjoint union
$F+G$ is the sum of the sizes $F$ and of $G$, and the size of a
product $F \cdot G$ is the product of the sizes.
...
trees, but the set of all binary trees is countably infinite.
