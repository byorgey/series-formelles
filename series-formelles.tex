...
\begin{commentary}
  ...
  even sets.  The point is that, via transport, $u : E \to F$ induces
  a bijection between the sets of structures $M[E]$ and $M[F]$.  If
  this bijection relates $s \in M[E]$ and $t \in M[F]$, then we say
  that $s$ and $t$ are isomorphic, as witnessed by $u$, and write
  $u : s \to t$.
