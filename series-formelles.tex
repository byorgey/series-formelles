...
\begin{commentary}
  ...
  for most purposes the distinction is not an important one.  In fact,
...
\begin{commentary}
  Recall that an endofunctor $M : \B \to \B$ has two components, an
  object mapping and a morphism mapping.  This first paragraph after
  the definition sets up some terminology related to the object
  mapping, which sends finite sets (that is, objects of $\B$) to finite
  sets.  In particular, one should think of $M$ as sending finite sets
  of \emph{labels} to finite sets of \emph{labelled structures}.

  For example, \todo{Give a simple example, with pictures.  Say how
    Joyal's terminology relates to the example \& pictures.}
\end{commentary}
...
\[ u : s \to t. \]
\begin{commentary}
  Second, there is a morphism mapping, sending morphisms to morphisms;
  in this case, it sends bijections between label sets
  (\term{relabellings}) to bijections between sets of labelled
  structures (with each structure corresponding to its relabelled
  version).

  $u : s \to t$ is a mild abuse of notation; $u$ is in fact a
  bijection between label sets $E$ and $F$, and $s$ and $t$ are not
  even sets.  The point is that, via transport, $u$ induces a
  bijection between the sets of structures $M[E]$ and $M[F]$.  If this
  bijection relates $s \in M[E]$ and $t \in M[F]$, then we say that
  $s$ and $t$ are isomorphic, as witnessed by $u$, and write $u : s
  \to t$.
...
There is a \term{forgetful} functor $U: \el (M) \to \B$ whose value on
$s \in M$ is the \term{underlying} set of the structure $s$. The
concept of isomorphism of structures defines an equivalence relation
whose classes are the \term{types} of structures of the species M;
these classes are the \term{connected components} of the groupoid $\el
