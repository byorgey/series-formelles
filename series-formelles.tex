...
cardinality of a finite class is just a single natural number;
generalizing to finitary classes, we can say that the ``cardinality''
of a finitary class is an \emph{infinite sequence} of natural numbers,
one for each natural number size (or, instead of a sequence, we can
equivalently think of it as a function $\N \to \N$).
